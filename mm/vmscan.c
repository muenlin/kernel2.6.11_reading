 ...
 */
 //但函数返回时，page_list中剩下的是无法回收的page
static int shrink_list(struct list_head *page_list, struct scan_control *sc)
{
	...
	while (!list_empty(page_list)) {
		...
		BUG_ON(PageActive(page));//?,where set?

		sc->nr_scanned++;
		/* Double the slab pressure for mapped and swapcache pages */
		if (page_mapped(page) || PageSwapCache(page))//why???
			sc->nr_scanned++;
...
static void shrink_cache(struct zone *zone, struct scan_control *sc)
{
	...
	while (max_scan > 0) {
		...
		spin_lock_irq(&zone->lru_lock);
		/*
		 * Put back any unfreeable pages.//当前page_list中存放的都是无法回收的page
		 */
		while (!list_empty(&page_list)) {
			page = lru_to_page(&page_list);
...
shrink_zone(struct zone *zone, struct scan_control *sc)
{
	...
	while (nr_active || nr_inactive) {
		if (nr_active) {
			...
			refill_inactive_zone(zone, sc);//补充管理区inactive_list
		}
		...
		if (nr_inactive) {
			...
			shrink_cache(zone, sc);//从inactive_list回收 page 
